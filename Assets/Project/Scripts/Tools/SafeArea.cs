...
namespace Crystal
{
    ...
    public class SafeArea : MonoBehaviour
    {
		...
		private int _nativeCutoutHeight;
		
		private void DebugSafeArea()
		{
			if (Application.platform == RuntimePlatform.Android)
			{
				try
				{
					...
					AndroidJavaClass safe = new AndroidJavaClass("com.playgenes.safearea.SafeAreaHelper");
					AndroidJavaObject androidSafe =
									safe.CallStatic<AndroidJavaObject>("getSafeArea", currentActivity);

					//int centerX = androidSafe.Call<int>("centerX");
					//int centerY = androidSafe.Call<int>("centerY");
					_nativeCutoutHeight = androidSafe.Call<int>("height");
					//_nativeCutoutWidth = androidSafe.Call<int>("width");

					Debug.Log($"[SafeArea] height = {_nativeCutoutHeight}");
				}
				catch (Exception ex)
				{
        ...
        void Refresh ()
        {
            ...
            if (safeArea != LastSafeArea
				...
				|| LastNativeCutoutHeight != _nativeCutoutHeight)
            {
		...
		public static Rect CustomPortraitSafeArea { get; private set; }

		Rect GetSafeArea ()
        {
            Rect safeArea = Screen.safeArea;
			
			if (safeArea.height >= Screen.height && Math.Abs(_nativeCutoutHeight) > 0)
				safeArea.height += _nativeCutoutHeight;
			...
			CustomPortraitSafeArea = safeArea;
            return safeArea;
        }
        ...
        void ApplySafeArea (Rect r)
        {
            ...
            if (Logging)
            {
                Debug.LogFormat ("New safe area applied to {0}: x={1}, y={2}, w={3}, h={4} on full extents w={5}, h={6}, nativeCutoutHeight={7}",
                name, r.x, r.y, r.width, r.height, Screen.width, Screen.height, LastNativeCutoutHeight);
            }
        }
    }
}
