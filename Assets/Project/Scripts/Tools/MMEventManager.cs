...
namespace Tools
{
    ...
    /// <summary>
    ///     This class handles event management, and can be used to broadcast events throughout the game, to tell one class (or
    ///     many) that something's happened.
    ...
    ///     For example : MMEventManager.TriggerEvent(new MMGameEvent("GameStart")); will broadcast an MMGameEvent named
    ///     GameStart to all listeners.
    ...
    ///     For example: public class GUIManager : Singleton
    ///     <GUIManager>
    ///         , MMEventListener
    ///         <MMGameEvent>
    ...
    ///                     }
    ///                     }
    ///                     will catch all events of type MMGameEvent emitted from anywhere in the game, and do something if
    ///                     it's named GameOver
    /// </summary>
    [ExecuteAlways]
    ...
    public static class EventRegister
    {
        ...
        public static void MMEventStartListening<EventType>(this MMEventListener<EventType> caller)
            where EventType : struct
        {
            MMEventManager.AddListener(caller);
        }

        public static void MMEventStopListening<EventType>(this MMEventListener<EventType> caller)
            where EventType : struct
        {
